...
import restate
from restate import VirtualObject, Workflow, ObjectContext, WorkflowContext
...
async def record_feedback(ctx: ObjectContext, feedback_data: dict) -> dict:
    ...
    await ctx.set("feedback_history", history)
    ...
    await ctx.set("stats", stats)
    
    return {"status": "recorded", "new_avg_rating": stats["avg_rating"]}


@feedback_store.handler()
async def get_user_preferences(ctx: ObjectContext) -> dict:
...
@feedback_store.handler()
async def get_feedback_history(ctx: ObjectContext) -> dict:
...
async def record_query_outcome(ctx: ObjectContext, data: dict) -> dict:
    ...
    await ctx.set("hop_groups", hop_groups)
...
@learning_store.handler()
async def get_learning_signals(ctx: ObjectContext) -> dict:
...
@learning_store.handler()
async def get_evolution_recommendations(ctx: ObjectContext) -> dict:
...
@search_agent.handler()
async def submit_feedback(ctx: WorkflowContext, feedback_data: dict) -> dict:
    """Handler for user feedback submission."""
    feedback = UserFeedback(**feedback_data)
    
    # Record in user's feedback store
    user_id = feedback.user_id or "anonymous"
    
    # Use the correct Restate SDK API - pass handler reference, not strings
    ctx.object_send(record_feedback, key=user_id, arg=asdict(feedback))
    
    # Get the original query info
    result_data = get_stored_result(feedback.query_id)
    ...
    if result_data:
        ...
        # Record in global learning store (HRPO-style) - use object_call for response
    ...
    return {"status": "feedback_recorded", "query_id": feedback.query_id}
...
app = restate.app(
    services=[search_agent, feedback_store, learning_store]
)
