...
3. Clinical explainability requirements
...
@dataclass
class ClinicalRetrievalConfig:
    """Configuration for clinical-grade retrieval"""
    # Lyapunov settings
    ...
    max_clinical_iterations: int = 3  # Lower for latency-sensitive clinical use
    ...
    def __post_init__(self):
        if self.lyapunov is None:
            self.lyapunov = LyapunovConfig(
                epsilon=0.02,
                max_iterations=self.max_clinical_iterations,
                min_iterations=1,
...
class LyapunovHRPOTrainer:
    ...
    def __init__(self, config: ClinicalRetrievalConfig):
        self.config = config
...
class ClinicalRetriever:
    """
    Clinical-grade retriever with Lyapunov stability guarantees.
    ...
    def __init__(
            ...
            rag_pipeline,
            query_expander,
            config: Optional[ClinicalRetrievalConfig] = None,
    ):
        self.rag = rag_pipeline
        self.expander = query_expander
        self.config = config or ClinicalRetrievalConfig()
    ...
    def search(
        ...
        """
        Perform clinical-grade search with Lyapunov guarantees.
        """
        ...
        result = {
            ...
            # Clinical flags
            "meets_clinical_requirements": meets_requirements,
...
def create_lyapunov_agent_handlers():
    ...
    async def stable_search(ctx: WorkflowContext, query_data: dict) -> dict:
        ...
        retriever = ClinicalRetriever(rag, expander)
...
def demo():
    """Demonstrate Lyapunov-stable clinical retrieval"""
    print("\n" + "=" * 70)
    print("LYAPUNOV-STABLE CLINICAL RETRIEVAL DEMO")
    print("=" * 70)
    ...
    clinical_config = ClinicalRetrievalConfig(
        min_confidence=0.5,  # Lower for demo
        require_convergence=False,  # Don't require for demo
        max_clinical_iterations=3,
    )

    retriever = ClinicalRetriever(rag, expander, clinical_config)

    # Clinical queries
    queries = [
        "What are the treatment guidelines for acute myocardial infarction?",
