...
stability guarantees. Critical for clinical applications where:
...
class RetrievalState:
    ...
    # Query state (maybe refined across iterations)
    original_query: str
    refined_query: str
...
class LyapunovRetriever:
    ...
    3. Full audit trail for clinical explainability
    """
    ...
    def get_audit_trail(self) -> List[Dict]:
        """Get full audit trail for clinical explainability"""
        trail = []
        for i, state in enumerate(self.state_history):
            ...
            if i > 0:
                ...
                entry["stability"] = "stable" if entry["delta_v"] <= 0 else "improving" if entry[
                                                                                               "delta_v"] < self.config.epsilon else "unstable"
...
def demo_lyapunov_retrieval():
    ...
    # Run retrieval
    query = "What are the clinical guidelines for treating acute myocardial infarction?"
    print(f"\nQuery: {query}")
    print("-" * 70)
